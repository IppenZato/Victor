...
import (
	...
	"strings"
	"sync"
	"github.com/Viktor19931/books_api/db/sqlx/reflectx"
	"time"
)
...
func (r *Row) Scan(dest ...interface{}) error {
	...
	for i, _ := range dest {
		src := refs[i]
		dst := dest[i]
		if err := setFieldValue(reflect.ValueOf(dst).Elem(), reflect.ValueOf(src).Elem().Interface() ); err != nil {
			return err
		}
	}
...
func setFieldValue(field reflect.Value, value interface{}) (err error) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		if value == nil {
			...
			default:
				...
				//crmlogs.Debug(fmt.Sprintf("[%v] = %v %+v %+v", val.Kind(), value, val.Type(), field.Type()))
				// Test as Duration
				s := ToStr(value)
				if s != "0" {
					dur, err := time.ParseDuration(s)
					if err == nil {
						//crmlogs.Notice(fmt.Sprintf("*[%v] = %v %+v %+v", val.Kind(), dur, val.Type(), field.Type()))
						field.SetInt(dur.Nanoseconds())
					}
				}
			}
		}
	...
	case reflect.Struct:
		...
		default:
			//if method := field.MethodByName("SetRaw"); method.IsValid() {
			//
			//	//fd := field.Addr().Interface().(Fielder)
			//	//err := fd.SetRaw(value)
			//
			//	err = method.Call([]reflect.Value{reflect.ValueOf(value)})
			//	if err != nil {
			//		return fmt.Errorf("converted value `%v` set to Fielder failed, err: %s", value, err)
			//	}
			//
			//}
			if field.CanAddr() {
				if fd, ok := field.Addr().Interface().(SetRawer); ok {
					err = fd.SetRaw(value)
					return err
				}
				err = fmt.Errorf("%v not processed\n", field.Kind())
			}
